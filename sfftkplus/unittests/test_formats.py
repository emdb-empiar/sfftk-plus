...
class TestFormats(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.roi_file = os.path.join(tests.TEST_DATA_PATH, 'test_emd_1832.roi')
        cls.sff_file = os.path.join(tests.TEST_DATA_PATH, 'test_emd_1832.sff')
    ...
    def test_from_vtk(self):
        ...
        args = parse_args(shlex.split('createroi {} -o file.roi'.format(os.path.join(tests.TEST_DATA_PATH, 'test_emd_1832.sff'))))
        vtk_segmentation = self.sff_segmentation.as_vtk(args)
        # to get contours we must first slice!
        vtk_segmentation.slice()
        roi_segmentation = vtk_segmentation.as_roi()
