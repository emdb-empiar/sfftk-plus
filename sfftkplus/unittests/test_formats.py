...
from sfftkplus.formats import roi
...
class TestFormats(unittest.TestCase):
    ...
    def test_from_vtk(self):
        ...
        args, configs = parse_args(shlex.split('createroi {} -o file.roi'.format(os.path.join(tests.TEST_DATA_PATH, 'test_emd_1832.sff'))))
        vtk_segmentation = self.sff_segmentation.as_vtk(args, configs)
        # to get contours we must first slice!
        vtk_segmentation.slice()
        roi_segmentation = vtk_segmentation.as_roi(configs)
