...
class TestParserROICreate(unittest.TestCase):
    ...
    def test_default(self):
        """Test default form"""
        args, _ = parse_args('roi create file.sff', use_shlex=True)
    ...
    def test_output(self):
        ...
        args, _ = parse_args('roi create -o fileAnother.roi file.sff', use_shlex=True)
        self.assertEqual(args.sff_file, 'file.sff')
    ...
    def test_overwrite(self):
        ...
        args, _ = parse_args('roi create -w file.sff', use_shlex=True)
        self.assertTrue(args.overwrite)

    def test_format(self):
        """Test that we can specify the output format only"""
        args, _ = parse_args('roi create -f json file.sff', use_shlex=True)
        self.assertEqual(args.sff_file, 'file.sff')
    ...
    def test_roi_file(self):
        ...
        args, _ = parse_args('roi create file.roi -o new_file.roi -I emd_1234 --reset-ids', use_shlex=True)
    ...
    def test_primary_descriptor(self):
        ...
        args, _ = parse_args('roi create file.sff -o file.roi -R three_d_volume', use_shlex=True)
        self.assertEqual(args.primary_descriptor, 'three_d_volume')
        args, _ = parse_args('roi create file.sff -o file.roi -R mesh_list', use_shlex=True)
        self.assertEqual(args.primary_descriptor, 'mesh_list')
        # args, _ = parse_args('roi create file.sff -o file.roi -R contourList', use_shlex=True)
        # self.assertEqual(args.primary_descriptor, 'contourList')
        args, _ = parse_args('roi create file.sff -o file.roi -R shape_primitive_list', use_shlex=True)
        self.assertEqual(args.primary_descriptor, 'shape_primitive_list')

    def test_wrong_primary_descriptor(self):
        """Test wrong primary descriptor"""
        args, _ = parse_args('roi create file.sff -o file.roi -R something', use_shlex=True)
        self.assertEqual(os.EX_DATAERR, args)

    def test_verbose(self):
        """Test verbose"""
        args, _ = parse_args('roi create file.sff -o file.roi -v', use_shlex=True)
        self.assertTrue(args.verbose)

    def test_transparency(self):
        """Test specifying transparency"""
        transparency = _random_float()
        args, _ = parse_args('roi create file.sff -o file.roi -t {}'.format(transparency), use_shlex=True)
    ...
    def test_image_name_root(self):
        ...
        args, _ = parse_args('roi create file.sff -o file.roi -I emd_1234', use_shlex=True)
        self.assertEqual(args.image_name_root, 'emd_1234')
        self.assertIsNone(args.top_front_right)

    def test_top_front_right(self):
        """Test that we can set the image IDs directly"""
        args, _ = parse_args('roi create file.sff -o file.roi --top-front-right 5 6 7', use_shlex=True)
    ...
    def test_quick_pick(self):
        ...
        args, _ = parse_args('roi create file.sff -o file.roi --quick-pick {}'.format(quick_pick), use_shlex=True)
        self.assertEqual(args.quick_pick, quick_pick - 1)

    def test_invalid_quick_pick_value(self):
        """Test that a --quick-pick <= 0 terminates"""
        args, _ = parse_args('roi create file.sff -o file.roi --quick-pick 0', use_shlex=True)
        self.assertEqual(os.EX_DATAERR, args)
        args, _ = parse_args('roi create file.sff -o file.roi --quick-pick -1', use_shlex=True)
        self.assertEqual(os.EX_DATAERR, args)

    def test_mask_value(self):
        """Test specifying mask value"""
        mask_value = _random_integer()
        args, _ = parse_args('roi create file.sff -o file.roi -m {}'.format(mask_value), use_shlex=True)
        self.assertEqual(args.mask_value, mask_value)

    def test_normals_off(self):
        """Test normals off"""
        args, _ = parse_args('roi create file.sff -o file.roi -N', use_shlex=True)
        self.assertTrue(args.normals_off)

    def test_config_path(self):
        """Test config path"""
        args, _ = parse_args('roi create file.sff -o file.roi --config-path {}'.format(self.config_fn), use_shlex=True)
        self.assertEqual(args.config_path, self.config_fn)
...
class TestParserROIAttach(unittest.TestCase):
    ...
    def test_default(self):
        """Test default form"""
        args, _ = parse_args('roi attach file.roi', use_shlex=True)
    ...
    def test_verbose(self):
        """Test verbose"""
        args, _ = parse_args('roi attach file.roi -v', use_shlex=True)
        self.assertTrue(args.verbose)

    def test_config_path(self):
        """Test config path"""
        args, _ = parse_args('roi attach file.roi --config-path {}'.format(self.config_fn), use_shlex=True)
        self.assertEqual(args.config_path, self.config_fn)

    def test_shipped_configs(self):
        """Test that we used shipped configs"""
        args, _ = parse_args('roi attach file.roi --shipped-configs', use_shlex=True)
        self.assertTrue(args.shipped_configs)

    def test_omero_local(self):
        '''Test that --local uses local omero'''
        args, configs = parse_args('roi attach file.roi --local', use_shlex=True)
        self.assertEqual(configs['CONNECT_WITH'], 'REMOTE')

    def test_omero_remote(self):
        '''Test that uses remote omero'''
        args, configs = parse_args('roi attach file.roi --remote', use_shlex=True)
        self.assertEqual(configs['CONNECT_WITH'], 'REMOTE')
...
class TestParserROIDel(unittest.TestCase):
    ...
    def test_default(self):
        ...
        args, _ = parse_args('roi del -r {}'.format(roi_id), use_shlex=True)
    ...
    def test_image_id(self):
        ...
        image_id = _random_integer()
        args, _ = parse_args('roi del -i {}'.format(image_id), use_shlex=True)
        self.assertEqual(args.image_id, image_id)

    def test_verbose(self):
        """Test verbose"""
        args, _ = parse_args('roi del -r {} -v'.format(_random_integer()), use_shlex=True)
        self.assertTrue(args.verbose)

    def test_config_path(self):
        """Test config path"""
        args, _ = parse_args('roi del -i {} --config-path {}'.format(_random_integer(), self.config_fn), use_shlex=True)
        self.assertEqual(args.config_path, self.config_fn)

    def test_shipped_configs(self):
        """Test that we use shipped configs"""
        args, _ = parse_args('roi del -i {} --shipped-configs'.format(_random_integer()), use_shlex=True)
        self.assertTrue(args.shipped_configs)

    def test_omero_local(self):
        '''Test that --local uses local omero'''
        args, configs = parse_args('roi del -i {} --local'.format(_random_integer()), use_shlex=True)
        self.assertEqual(configs['CONNECT_WITH'], 'REMOTE')

    def test_omero_remote(self):
        '''Test that uses remote omero'''
        args, configs = parse_args('roi del -i {} --remote'.format(_random_integer()), use_shlex=True)
        self.assertEqual(configs['CONNECT_WITH'], 'REMOTE')
...
class TestParserView3D(unittest.TestCase):
    ...
    def test_default(self):
        """Test default form"""
        args, _ = parse_args('view --visualise file.sff', use_shlex=True)
        print(args)
    ...
    def test_background_colour(self):
        ...
        args, _ = parse_args('view --visualise file.sff -B {} {} {}'.format(r, g, b), use_shlex=True)
    ...
    def test_cube_axes(self):
        ...
        args, _ = parse_args('view --visualise file.sff -c {}'.format(cube_axes), use_shlex=True)
        self.assertEqual(args.cube_axes, cube_axes)

    def test_view_edges(self):
        """Test specifying to view edges"""
        args, _ = parse_args('view --visualise file.sff -e', use_shlex=True)
    ...
    def test_normals_off(self):
        ...
        args, _ = parse_args('view --visualise file.sff -N', use_shlex=True)
        self.assertTrue(args.normals_off)

    def test_wireframe(self):
        """Test specifying display as wireframe """
        args, _ = parse_args('view --visualise file.sff -w', use_shlex=True)
    ...
    def test_transparency(self):
        """Test specifying transparency"""
        transparency = _random_float()
        args, _ = parse_args('view --visualise file.sff -t {}'.format(transparency), use_shlex=True)
        self.assertEqual(round(args.transparency, 6), round(transparency, 6))

    def test_verbose(self):
        """Test specifying verbose"""
        args, _ = parse_args('view --visualise file.sff -v', use_shlex=True)
        self.assertTrue(args.verbose)

    def test_mask_value(self):
        """Test specifying mask value"""
        mask_value = _random_integer()
        args, _ = parse_args('view --visualise file.sff -m {}'.format(mask_value), use_shlex=True)
        self.assertEqual(args.mask_value, mask_value)

    def test_config_path(self):
        """Test that we can set a config path"""
        args, _ = parse_args('view --visualise file.sff --config-path {}'.format(self.config_fn), use_shlex=True)
        self.assertEqual(args.config_path, self.config_fn)
